...
const USER_AGENT = "UniPathAustralia/0.4 (+https://github.com/bkushen/unipath-australia; official university course-link verification)";
...
function sameUniversityHost(url, allowedHosts) {
  ...
  return [...allowedHosts].some((allowed) => { const cleaned = allowed.toLowerCase().replace(/^www\./, ""); return host === cleaned || host.endsWith(`.${cleaned}`) || cleaned.endsWith(`.${host}`); });
}
...
async function fetchCoursesForUniversity(universityId) {
  ...
  for (let from = 0; ; from += pageSize) {
    ...
    if (error) throw error; if (!data?.length) break; rows.push(...data); if (data.length < pageSize) break;
  }
  return rows;
}
...
function verifyPage(course, url, html) {
  ...
  const score = Math.max(0, Math.min(1, tokenCoverage(course.name, headline) * .42 + tokenJaccard(course.name, headline) * .20 + tokenCoverage(course.name, pageText) * .14 + identifierBonus + (exactHeadline ? .08 : 0) - (conflicts.length ? .20 : 0)));
  return { title, h1, score, exactHeadline, courseCodeFound, cricosFound, vetCodeFound, conflicts };
}
...
async function verifyCourse(course, candidateUrls, allowedHosts) {
  ...
  for (const [kind, key] of [["CRICOS","cricosFound"],["VET","vetCodeFound"],["UNIVERSITY_COURSE_CODE","courseCodeFound"]]) {
    const pages = evaluated.filter((item) => item[key] && item.confidence >= Math.max(.72, minimumConfidence - .16));
    const distinct = [...new Set(pages.map((item) => item.url))];
    if (distinct.length > 1) { collisionKinds.push(kind); distinct.forEach((url) => collisionUrls.add(url)); }
  }
...
for (const university of universities) {
  ...
  const headers = ["university","course_id","course_name","cricos_code","university_course_code","vet_national_code","duplicate_name_count","previous_url","candidate_url","confidence","title","h1","cricos_found","course_code_found","vet_code_found","exact_headline","specialisation_conflicts","identifier_collision_kinds","identifier_collision_urls","accepted","rejection_reason","write_status"];
