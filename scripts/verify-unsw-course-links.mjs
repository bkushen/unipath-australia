...
const profileDir = join(tmpdir(), "unipath-unsw-verify-chrome-profile");
...
function extractCricos(text) { return [...new Set([...String(text ?? "").matchAll(/\bCRICOS\s+Code\s*[:#-]?\s*([0-9]{6}[A-Z]|[0-9]{7})\b/gi)].map((m) => m[1].toUpperCase()))]; }
function extractProgrammeName(text) {
  const lines = String(text ?? "").split(/\r?\n/).map((x) => x.trim()).filter(Boolean);
  const homeIndex = lines.findIndex((line) => /^Home$/i.test(line));
  if (homeIndex >= 0) {
    const candidates = lines.slice(homeIndex + 1, homeIndex + 8).filter((line) => !/^(Undergraduate|Postgraduate|Program|Programs|2026)$/i.test(line));
    if (candidates[0]) return candidates[0];
  }
  const heading = lines.find((line) => /^(Bachelor|Master|Doctor|Graduate|Diploma|Juris|Psychology|Engineering|Commerce|Design|Education|Science|Arts|Economics|Translation|Interpreting|Financial|Global|Public|Simulation|Teaching)/i.test(line));
  return heading ?? "";
}
function extractAwards(text) {
  const section = String(text ?? "").match(/Award\(s\)[\s\S]{0,1400}?(?=\n(?:UAC Code|CRICOS Code|Learning Outcomes|Program Structure|Overview)\b)/i)?.[0] ?? "";
  return [...new Set([...section.matchAll(/\b((?:Bachelor|Master|Doctor|Graduate Certificate|Graduate Diploma|Diploma|Associate Degree|Juris Doctor)[^\n]{2,180}?)(?=\s+-\s+[A-Z]|\n|$)/g)].map((m) => m[1].replace(/\s+/g, " ").trim()).filter(Boolean))];
}
...
async function renderPage(cdp, url) {
  await cdp.send("Page.navigate", { url });
  for (let attempt = 0; attempt < 30; attempt += 1) {
    await sleep(400);
    const state = await cdp.eval(`(() => ({href: location.href, title: document.title || "", text: document.body?.innerText || ""}))()`);
    if (state?.href?.includes("handbook.unsw.edu.au") && state.text?.length > 500) return state;
  }
...
try {
  for (let index = 0; index < accepted.length; index += 1) {
    ...
    try {
      ...
      const page = await renderPage(browser.cdp, row.candidate_url);
      if (programmeCodeFromUrl(page.href) !== expectedCode) throw new Error("rendered_program_code_mismatch");
      pageProgrammeName = extractProgrammeName(page.text);
      pageAwards = extractAwards(page.text);
      pageCricos = extractCricos(page.text);
