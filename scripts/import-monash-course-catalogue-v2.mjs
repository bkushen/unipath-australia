...
const cataloguePath = `${outputDir}/monash-official-catalogue-v2.json`;
const auditJsonPath = `${outputDir}/monash-catalogue-match-v2.json`;
const auditCsvPath = `${outputDir}/monash-catalogue-match-v2.csv`;
const SCHEMA_VERSION = 2;
const MONASH_ROOT = "https://www.monash.edu";
const MONASH_INDEX = `${MONASH_ROOT}/study/courses/find-a-course`;
const USER_AGENT = "UniPathAustralia/1.1 (+https://github.com/bkushen/unipath-australia; Monash official course catalogue verification)";
...
function resolveCourseHref(rawHref) {
  ...
  try {
    ...
    if (parsed.hostname === "mon-search.funnelback.squiz.cloud") {
      ...
      const canonical = canonicalCourseUrl(decoded);
...
async function fetchText(url) {
  const response = await fetch(url, { redirect: "follow", headers: { "user-agent": USER_AGENT, accept: "text/html,application/xhtml+xml,*/*" }, signal: AbortSignal.timeout(25000) });
  if (!response.ok) throw new Error(`HTTP ${response.status} for ${url}`);
  return { text: await response.text(), finalUrl: response.url };
}

async function discoverCourseUrls() {
  const pages = [MONASH_INDEX, `${MONASH_INDEX}?international=true`];
  const urls = new Set();
...
async function buildOfficialCatalogue() {
  console.log("Building Monash official catalogue from Funnelback-backed Find a Course results...");
  ...
  for (let index = 0; index < courseUrls.length; index += 1) {
    ...
    try {
      const internationalUrl = `${rootUrl}?international=true`;
      const { text } = await fetchText(internationalUrl);
      const title = extractTag(text, "title");
      const h1 = extractTag(text, "h1");
      const pageText = stripHtml(text).slice(0, 240000);
      const displayName = h1 || cleanTitle(title);
      entries.push({ url: rootUrl, providerCode: monashCourseCode(rootUrl), title, h1, displayName, cricosCodes: labelledCricos(pageText) });
    } catch (error) {
      console.warn(`  course page skipped: ${rootUrl} (${error.message})`);
    }
  ...
  await writeFile(cataloguePath, `${JSON.stringify({ schemaVersion: SCHEMA_VERSION, generatedAt: new Date().toISOString(), source: MONASH_INDEX, entries: catalogue }, null, 2)}\n`, "utf8");
...
function rankCandidate(course, entry) {
  const exactName = normaliseText(course.name) === normaliseText(entry.displayName) || normaliseText(entry.displayName).includes(normaliseText(course.name));
  ...
  let score = coverage * 0.42 + jaccard * 0.26 + (exactName ? 0.20 : 0) + (cricosMatch ? 0.36 : 0) + (providerMatch ? 0.34 : 0);
  score = Math.min(1, score);
  if (exactName && cricosMatch) score = 1;
  else if (exactName && providerMatch) score = 1;
  else if (exactName && coverage >= 0.95) score = Math.max(score, 0.94);
...
console.log("\n=== Monash catalogue-first matching v2 ===");
...
for (let index = 0; index < selected.length; index += 1) {
  ...
  auditRows.push({ course_id: course.id, course_name: course.name, cricos_code: course.cricos_code, duplicate_name_count: duplicateNameCounts.get(normaliseText(course.name)) ?? 1, candidate_url: best?.entry.url ?? null, candidate_course_code: best?.entry.providerCode ?? null, candidate_name: best?.entry.displayName ?? null, candidate_cricos_codes: best?.entry.cricosCodes?.join("|") ?? "", confidence: best ? Number(best.score.toFixed(4)) : null, exact_name: best?.exactName ?? false, cricos_match: best?.cricosMatch ?? false, provider_code_match: best?.providerMatch ?? false, accepted: result.accepted, rejection_reason: result.reason, write_status: writeStatus });
...
const headers = ["course_id","course_name","cricos_code","duplicate_name_count","candidate_url","candidate_course_code","candidate_name","candidate_cricos_codes","confidence","exact_name","cricos_match","provider_code_match","accepted","rejection_reason","write_status"];
...
if (!writeMode) console.log("Dry run only. Monash course pages are discovered from Funnelback redirect targets and duplicate names require CRICOS or course-code evidence.");
