...
const cataloguePath = `${outputDir}/sydney-course-catalogue.json`;
const auditJson = `${outputDir}/sydney-course-match.json`;
const auditCsv = `${outputDir}/sydney-course-match.csv`;
const UNIVERSITY_NAME = "University of Sydney";
const ROOT = "https://www.sydney.edu.au";
...
function normalise(value) {
  return String(value ?? "")
    ...
    .replace(/\bachelor of /gi, "bachelor of ")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
    .trim();
...
function isCourseUrl(url) {
  return /^https:\/\/www\.sydney\.edu\.au\/(?:content\/)?courses\/courses\/(?:uc|pc)\/[^?#]+\.html(?:[?#].*)?$/i.test(url);
}
...
async function discoverCourseUrls() {
  ...
  while (queue.length) {
    ...
    for (const loc of sitemapLocs(xml)) {
      if (/\.xml(?:\?|$)/i.test(loc) && loc.startsWith(ROOT) && !seenSitemaps.has(loc)) queue.push(loc);
      else if (isCourseUrl(loc)) courseUrls.add(loc.replace("/content/courses/", "/courses/"));
    }
    if (seenSitemaps.size > 250) break;
  }

  // Sydney also exposes a public all-links page. Use it as a safe discovery fallback.
  if (!courseUrls.size) {
    try {
      const html = await fetchText(`${ROOT}/courses/all-links-page.html`);
      for (const match of html.matchAll(/https:\/\/www\.sydney\.edu\.au\/courses\/courses\/(?:uc|pc)\/[^"'<>\s]+?\.html/gi)) {
        if (isCourseUrl(match[0])) courseUrls.add(match[0]);
      }
    } catch (error) {
      console.warn(`Could not read Sydney all-links fallback: ${error.message}`);
    }
  }

  return [...courseUrls].sort();
}
...
function extractPageMetadata(html, url) {
  ...
  const name = h1 || ogTitle.replace(/\s*-\s*The University of Sydney\s*$/i, "").trim() || title;
  ...
  const courseCodes = [...new Set([
    ...text.matchAll(/\b(?:course\s+code|course code)\s*[:#-]?\s*([A-Z][A-Z0-9-]{4,20})\b/gi),
    ...text.matchAll(/\b([A-Z]{2,8}[A-Z0-9]{3,14}-\d{2})\b/g),
  ].map((m) => m[1]?.toUpperCase()).filter(Boolean))];
  const level = /\/uc\//i.test(url) ? "undergraduate" : "postgraduate";
  return { url, name, normalisedName: normalise(name), cricosCodes, courseCodes, level, pageVerified: Boolean(name && name.length > 3) };
}
...
console.log(`Discovering official University of Sydney course pages...`);
const urls = await discoverCourseUrls();
if (!urls.length) throw new Error("No University of Sydney course URLs were discovered from official sources.");
console.log(`Discovered ${urls.length} candidate official course pages.`);
...
for (let index = 0; index < urls.length; index += 1) {
  ...
  if ((index + 1) % 50 === 0) console.log(`Fetched ${index + 1}/${urls.length} course pages...`);
}
...
for (const course of work) {
  ...
  if (dbCricos && (byCricos.get(dbCricos)?.length ?? 0) === 1) {
    ...
    rejectionReason = "no_exact_current_course_match";
  }
  ...
  rows.push({
    ...
    candidate_course_codes: candidate?.courseCodes ?? [],
  });
}

await writeFile(auditJson, `${JSON.stringify({ generatedAt: new Date().toISOString(), writeMode, rows }, null, 2)}\n`, "utf8");
const headers = ["course_id","course_name","cricos_code","university_course_code","accepted","match_method","rejection_reason","candidate_url","candidate_name","candidate_cricos_codes","candidate_course_codes"];
...
console.log("\n=== University of Sydney course-link run summary ===");
console.log(JSON.stringify({
  databaseCourses: courses?.length ?? 0,
