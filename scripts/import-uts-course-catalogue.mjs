...
const cataloguePath = `${outputDir}/uts-course-catalogue.json`;
const auditJson = `${outputDir}/uts-course-match.json`;
const auditCsv = `${outputDir}/uts-course-match.csv`;
const UNIVERSITY_NAME = "University of Technology Sydney";
const ROOT = "https://www.uts.edu.au";
...
async function discoverCourseUrls() {
  ...
  while (queue.length) {
    ...
    for (const loc of sitemapLocs(xml)) {
      if (/\.xml(?:\?|$)/i.test(loc) && /^https:\/\/(?:www\.)?uts\.edu\.au\//i.test(loc) && !seen.has(loc)) {
        queue.push(loc);
        continue;
      }
      const courseUrl = canonicalCourseUrl(loc);
      if (courseUrl) urls.add(courseUrl);
    }
...
function extractPageMetadata(html, url) {
  ...
  return { url, name, normalisedName: normalise(name), cricosCodes, courseCodes, pageVerified: Boolean(name && name.length > 3) };
}
...
console.log("Discovering official UTS course pages...");
const urls = await discoverCourseUrls();
if (!urls.length) throw new Error("No UTS course URLs were discovered from the official sitemap.");
console.log(`Discovered ${urls.length} candidate course URLs.`);
...
for (let index = 0; index < urls.length; index += 1) {
  ...
  try {
    ...
    const finalCourseUrl = canonicalCourseUrl(page.finalUrl);
    if (!finalCourseUrl) continue;
    const metadata = extractPageMetadata(page.text, finalCourseUrl);
