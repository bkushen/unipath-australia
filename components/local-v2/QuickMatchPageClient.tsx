...
function ResultCards({ items, highestQualification, comparisonIds, setComparisonIds, semesterBudget, fullCourseBudget }: { items: LiveRecommendation[]; highestQualification: string; comparisonIds: string[]; setComparisonIds: ComparisonSetter; semesterBudget: number; fullCourseBudget: number }) {
  ...
  return <div style={{ marginTop: 16 }}><div style={info}><strong>Compare courses:</strong> select up to 3 recommendations. {comparisonIds.length}/3 selected.</div><div style={{ display: "grid", gap: 16, marginTop: 14 }}>{items.slice(0, 8).map((item, index) => { const scoreValue = item.ai?.aiScore ?? item.scores.overall; const selectedForCompare = comparisonIds.includes(item.course.id); const universityCourseUrl = item.course.officialCourseUrl ?? item.university.website; const exactCourseUrlLoaded = Boolean(item.course.officialCourseUrl); return <article key={item.course.id} style={{ ...card, ...(selectedForCompare ? { borderColor: "#73aaf5", borderWidth: 2 } : {}) }}><div style={row}><div><div style={eyebrow}>#{index + 1} {index === 0 ? "BEST MATCH" : "ALTERNATIVE"}</div><h3 style={{ margin: "6px 0" }}>{item.course.name}</h3><strong style={{ color: "#0057b8" }}>{item.university.name}</strong><div style={muted}>{item.campus.name}{item.campus.city ? ` · ${item.campus.city}` : ""}{item.campus.state ? `, ${item.campus.state}` : ""}</div></div><div style={score}>{scoreValue}%</div></div>
        ...
        {!exactCourseUrlLoaded && item.university.website && <div style={courseLinkNote}><strong>Exact course-page link not yet verified.</strong> The university website link is provided so UniPath never fabricates a course URL.</div>}
        <div style={pills}><Link href={`/local-v2/courses/${item.course.id}`} style={linkButton}>View course details</Link><Link href={`/local-v2/universities/${item.university.id}`} style={secondaryLink}>University profile</Link>{universityCourseUrl && <a href={universityCourseUrl} target="_blank" rel="noreferrer" style={exactCourseUrlLoaded ? officialCourseLink : secondaryLink}>{exactCourseUrlLoaded ? "Official university course page ↗" : "University website ↗"}</a>}</div>
...
function Comparison({ items }: { items: LiveRecommendation[] }) {
  const comparisonRows: Array<[string, (item: LiveRecommendation) => string]> = [
    ...
    ["University course website", (item) => item.course.officialCourseUrl ? "Official course page loaded" : item.university.website ? "University website fallback" : "Not loaded"],
  ];
...
const officialCourseLink = { ...secondaryLink, background: "#ecfdf3", borderColor: "#abefc6", color: "#067647" } as const;
const courseLinkNote = { marginTop: 12, padding: 10, borderRadius: 9, background: "#fffaeb", border: "1px solid #fedf89", color: "#7a4d00", fontSize: 12, lineHeight: 1.45 } as const;
