...
export default function QuickMatchPageClient() {
  ...
  const retry = () => lastRequest && loadRecommendations(lastRequest);
...
function ResultSection({ title, loading, error, retry, items, profile, aiMessage, comparisonIds, setComparisonIds, onEdit, semesterBudget, fullCourseBudget, migration = false }: { title: string; loading: boolean; error: string; retry: () => void; items: LiveRecommendation[]; profile: StudentDecisionProfile; aiMessage: string; comparisonIds: string[]; setComparisonIds: React.Dispatch<React.SetStateAction<string[]>> | (() => void); onEdit: (step: QuickStep) => void; semesterBudget: number; fullCourseBudget: number; migration?: boolean }) {
  if (loading) return <section style={panel}><Loading /></section>;
...
function Loading() { return <div role="status" aria-live="polite" aria-busy="true" style={loading}><div style={spinner} /><div><strong>Finding your best matches…</strong><div style={muted}>Checking live courses, career alignment, tuition, location and available evidence.</div></div></div>; }

function ResultCards({ items, highestQualification, comparisonIds, setComparisonIds, semesterBudget, fullCourseBudget }: { items: LiveRecommendation[]; highestQualification: string; comparisonIds: string[]; setComparisonIds: React.Dispatch<React.SetStateAction<string[]>> | (() => void); semesterBudget: number; fullCourseBudget: number }) {
  const selectedItems = useMemo(() => items.filter((x) => comparisonIds.includes(x.course.id)).slice(0, 3), [items, comparisonIds]);
  const toggle = (id: string) => { if (typeof setComparisonIds !== "function") return; (setComparisonIds as React.Dispatch<React.SetStateAction<string[]>>)((current) => current.includes(id) ? current.filter((x) => x !== id) : current.length >= 3 ? current : [...current, id]); };
  return <div style={{ marginTop: 16 }}><div style={info}><strong>Compare courses:</strong> select up to 3 recommendations. {comparisonIds.length}/3 selected.</div><div style={{ display: "grid", gap: 16, marginTop: 14 }}>{items.slice(0, 8).map((item, index) => { const score = item.ai?.aiScore ?? item.scores.overall; const selectedForCompare = comparisonIds.includes(item.course.id); return <article key={item.course.id} style={{ ...card, ...(selectedForCompare ? { borderColor: "#73aaf5", borderWidth: 2 } : {}) }}><div style={row}><div><div style={eyebrow}>#{index + 1} {index === 0 ? "BEST MATCH" : "ALTERNATIVE"}</div><h3 style={{ margin: "6px 0" }}>{item.course.name}</h3><strong style={{ color: "#0057b8" }}>{item.university.name}</strong><div style={muted}>{item.campus.name}{item.campus.city ? ` · ${item.campus.city}` : ""}{item.campus.state ? `, ${item.campus.state}` : ""}</div></div><div style={score}>{score}%</div></div>
...
function Comparison({ items }: { items: LiveRecommendation[] }) { return <div style={comparison}><h3>Compare selected courses</h3><div style={{ overflowX: "auto" }}><table style={{ width: "100%", minWidth: 700, borderCollapse: "collapse" }}><thead><tr><th style={cell}>Factor</th>{items.map((x) => <th key={x.course.id} style={cell}>{x.course.name}<div style={muted}>{x.university.name}</div></th>)}</tr></thead><tbody>{[["Match score", (x: LiveRecommendation) => `${x.ai?.aiScore ?? x.scores.overall}%`],["Career fit", (x: LiveRecommendation) => `${x.scores.career}%`],["Budget fit", (x: LiveRecommendation) => `${x.scores.affordability}%`],["Location fit", (x: LiveRecommendation) => `${x.scores.location}%`],["Annual tuition", (x: LiveRecommendation) => money(x.course.annualFee,x.course.currency)],["Total tuition", (x: LiveRecommendation) => money(x.course.totalFee,x.course.currency)],["Entry evidence", (x: LiveRecommendation) => x.ai?.entryEvidence?.label ?? "Not loaded"],["Scholarship", (x: LiveRecommendation) => x.scholarship ? `Linked: ${x.scholarship.name}` : "No linked scholarship"]] as [string,(x:LiveRecommendation)=>string][] .map(([name, fn]) => <tr key={name}><th style={cell}>{name}</th>{items.map((x) => <td key={x.course.id} style={cell}>{fn(x)}</td>)}</tr>)}</tbody></table></div><div style={warning}>Comparison supports decisions only. It does not prove admission, scholarship eligibility, registration, visa eligibility or PR outcomes.</div></div>; }
...
const loading = { minHeight: 230, display: "flex", alignItems: "center", justifyContent: "center", gap: 18 } as const;
