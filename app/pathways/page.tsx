...
type Program={id:string;subclass:string;name:string;stream:string|null;pathway_type:string|null;source_url:string|null;verified_at:string|null;skilled_occupation_programs:Array<{skilled_occupations:{id:string;name:string;assessing_authority:string|null;skilled_occupation_codes:Array<{anzsco_code:string;anzsco_version:string}>}|null}>};

export default async function PathwaysPage({searchParams}:{searchParams:Promise<SearchParams>}){
  const raw=await searchParams; const q=(raw.q??"").trim().slice(0,80); const supabase=await createClient();
  let query=supabase.from("migration_programs").select(`id,subclass,name,stream,pathway_type,source_url,verified_at,skilled_occupation_programs(skilled_occupations(id,name,assessing_authority,skilled_occupation_codes(anzsco_code,anzsco_version)))`).order("subclass");
  if(q) query=query.or(`subclass.ilike.%${q}%,name.ilike.%${q}%,stream.ilike.%${q}%`);
  const {data,error}=await query; const programs=(data??[]) as Program[];
