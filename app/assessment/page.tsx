    ...
    <main className="assessmentPage">
      ...
      <section className="assessmentWrap shell">
        ...
        <section className="formPanel">
          ...
          <div className="formActions"><button className="back" disabled={step===0} onClick={()=>setStep(s=>Math.max(0,s-1))}><ArrowLeft size={17}/> Back</button>{step < steps.length-1 ? <button className="button" onClick={()=>setStep(s=>Math.min(steps.length-1,s+1))}>Continue <ArrowRight size={17}/></button> : <button className="button" onClick={finish}>Generate recommendations <ArrowRight size={17}/></button>}</div>
        </section>
      </section>
    </main>
  );
