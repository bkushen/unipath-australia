...
export default function QuickMatchPage() {
  ...
  const editAnswers = (targetStep: QuickStep) => {
  ...
  const cancelEditing = () => {
    ...
    setValidationError("");
    setError("");
    setStage(results.length ? "result" : "input");
  };
  ...
  const loadRecommendations = async (migrationImportance: MigrationImportance, target: "standard" | "migration", detailedMode = false) => {
    ...
    try {
      ...
      if (target === "standard") {
        setResults(scored);
        setComparisonIds((current) => current.filter((id) => scored.some((item) => item.course.id === id)));
      } else setMigrationResults(scored);
    } catch (err) { setError((err as Error).message); } finally { setLoading(false); }
  };

  const showQuickResult = async () => { setValidationError(""); setEditingFromResults(false); setStage("result"); await loadRecommendations("none", "standard", false); };
  ...
  const reset = () => { clearLocalV2Profile(); setProfile(initialProfile); setStage("input"); setStep(1); setResults([]); setMigrationResults([]); setComparisonIds([]); setEditingFromResults(false); setError(""); setValidationError(""); };
  ...
  return <main style={pageStyle}>
    ...
    {stage === "input" && <section style={shellStyle}>
      {editingFromResults && results.length > 0 && <div style={editBannerStyle}><div><strong>Editing your saved Quick Match answers</strong><div style={qualificationNoteStyle}>Your current results are kept while you edit. They will be replaced only when you press Get Match Score again.</div></div><button type="button" onClick={cancelEditing} style={secondaryButtonStyle}>Cancel editing</button></div>}
      ...
      {validationError && <div style={errorStyle}>{validationError}</div>}<div style={footerStyle}><button type="button" disabled={step === 1} onClick={goBack} style={secondaryButtonStyle}>← Back</button>{step < 4 ? <button type="button" onClick={goNext} style={primaryButtonStyle}>Continue →</button> : <button type="button" onClick={showQuickResult} style={primaryButtonStyle}>{editingFromResults ? "Recalculate Match Score →" : "Get Match Score →"}</button>}</div></section>}

    {(stage === "result" || stage === "detailed-result") && <><section style={sectionStyle}>{loading ? <LoadingPanel /> : <><div style={qualificationTopStyle}><div><div style={eyebrowStyle}>{stage === "detailed-result" ? "DETAILED SMART-SCORED RESULT" : "SMART-SCORED LIVE RESULT"}</div><h2 style={{ marginBottom: 0 }}>{stage === "detailed-result" ? "Your detailed best matches" : "Your best matches"}</h2></div><button type="button" onClick={() => editAnswers(1)} style={secondaryButtonStyle}>Edit answers</button></div><p style={mutedStyle}>{stage === "detailed-result" ? "The detailed score adds transparent planning adjustments for entered funds, relevant experience, skill overlap and dependants. It still does not decide admission or visa eligibility." : "The match score is decision support, not an admission guarantee. Missing course requirements are shown as unverified rather than guessed."}</p><div style={editShortcutStyle}><span style={editShortcutLabelStyle}>Recalculate after changing:</span><button type="button" onClick={() => editAnswers(1)} style={editShortcutButtonStyle}>Background & career</button><button type="button" onClick={() => editAnswers(2)} style={editShortcutButtonStyle}>Budget</button><button type="button" onClick={() => editAnswers(3)} style={editShortcutButtonStyle}>Location</button><button type="button" onClick={() => editAnswers(4)} style={editShortcutButtonStyle}>Optional details</button></div>{aiMessage && <div style={infoBannerStyle}><strong>{aiMode === "openai" ? "Optional AI scoring active" : "Free transparent scoring"}:</strong> {aiMessage}</div>}{error ? <ErrorBox text={error} /> : <ResultCards results={results} highestQualification={profile.highestQualification} semesterBudget={semesterBudget} fullCourseBudget={fullCourseBudget} comparisonIds={comparisonIds} onToggleComparison={toggleComparison} />}</>}</section>{stage === "result" && !loading && !error && <section style={{ ...sectionStyle, marginTop: 16 }}><h2>Want a more detailed result?</h2><p style={mutedStyle}>Add funds, experience, skills and dependants without losing your Quick Match answers.</p><button type="button" onClick={() => setStage("detailed")} style={primaryButtonStyle}>Continue to Detailed Assessment</button></section>}{!loading && !error && <MigrationPrompt choice={migrationChoice} setChoice={setMigrationChoice} onContinue={showMigrationResult} />}</>}
...
function ResultCards({ results, highestQualification, semesterBudget, fullCourseBudget, comparisonIds = [], onToggleComparison }: { results: LiveRecommendation[]; highestQualification: string; semesterBudget: number; fullCourseBudget: number; comparisonIds?: string[]; onToggleComparison?: (courseId: string) => void }) {
  ...
  return <>
    ...
    {onToggleComparison && comparisonIds.length === 1 && <div style={compareHintStyle}>Select one more course to open the side-by-side comparison.</div>}
    {onToggleComparison && selectedForComparison.length >= 2 && <CourseComparisonPanel items={selectedForComparison} onRemove={onToggleComparison} />}
  </>;
}

function CourseComparisonPanel({ items, onRemove }: { items: LiveRecommendation[]; onRemove: (courseId: string) => void }) {
  const row = (label: string, value: (item: LiveRecommendation) => string) => <div style={comparisonRowStyle}><strong style={comparisonLabelStyle}>{label}</strong>{items.map((item) => <span key={`${item.course.id}-${label}`} style={comparisonCellStyle}>{value(item)}</span>)}</div>;
  return <section style={comparisonPanelStyle} aria-labelledby="quick-match-comparison-heading"><div style={qualificationTopStyle}><div><div style={eyebrowStyle}>SIDE-BY-SIDE DECISION VIEW</div><h3 id="quick-match-comparison-heading" style={{ margin: "5px 0 0", fontSize: 24 }}>Compare selected courses</h3></div><span style={qualificationBadgeStyle}>{items.length}/3 courses</span></div><p style={mutedStyle}>Compare the evidence currently loaded in UniPath. Missing values stay marked as not loaded rather than being estimated.</p><div style={{ overflowX: "auto" }}><div style={{ minWidth: Math.max(720, 230 + items.length * 260) }}><div style={comparisonHeaderStyle}><strong style={comparisonLabelStyle}>Course</strong>{items.map((item) => <div key={item.course.id} style={comparisonCourseHeaderStyle}><strong>{item.course.name}</strong><span>{item.university.name}</span><button type="button" onClick={() => onRemove(item.course.id)} style={removeCompareButtonStyle}>Remove</button></div>)}</div>{row("Match score", (item) => `${item.ai?.aiScore ?? item.scores.overall}%`)}{row("Qualification", (item) => item.course.qualificationLevel ?? "Not loaded")}{row("Career fit", (item) => `${item.scores.career}%`)}{row("Course relevance", (item) => `${item.scores.academic}%`)}{row("Budget fit", (item) => `${item.scores.affordability}%`)}{row("Location fit", (item) => `${item.scores.location}%`)}{row("Campus", (item) => `${item.campus.name}${item.campus.city ? ` · ${item.campus.city}` : ""}${item.campus.state ? `, ${item.campus.state}` : ""}`)}{row("Duration", (item) => item.course.durationMonths ? `${item.course.durationMonths} months` : "Not loaded")}{row("Annual tuition", (item) => money(item.course.annualFee, item.course.currency))}{row("Total tuition", (item) => money(item.course.totalFee, item.course.currency))}{row("Living cost", (item) => item.livingCost ? `${money(item.livingCost.weeklyLow)}–${money(item.livingCost.weeklyHigh)}/week` : "Not loaded")}{row("Entry evidence", (item) => item.ai?.entryEvidence?.label ?? "Requirements not loaded")}{row("Scholarship", (item) => item.scholarship ? `Linked: ${item.scholarship.name}` : "No linked scholarship")}</div></div><div style={warningStyle}>A stronger comparison score does not prove admission, scholarship eligibility, professional registration, migration eligibility or PR outcomes. Confirm final requirements and current fees with official sources before applying.</div></section>;
}
...
const editBannerStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, flexWrap: "wrap", marginBottom: 18, padding: 14, border: "1px solid #b2ddff", borderRadius: 12, background: "#f5fbff" } as const;
const editShortcutStyle = { display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", margin: "12px 0 4px", padding: 10, border: "1px solid #e1e6ed", borderRadius: 10, background: "#f8fafc" } as const;
const editShortcutLabelStyle = { color: "#667085", fontSize: 12, fontWeight: 750, marginRight: 2 } as const;
const editShortcutButtonStyle = { border: "1px solid #cfd5df", background: "#fff", color: "#344054", borderRadius: 999, padding: "6px 10px", fontSize: 12, fontWeight: 800, cursor: "pointer" } as const;
...
const selectedCompareCardStyle = { border: "2px solid #73aaf5", background: "#f7fbff" } as const;
const compareToolbarStyle = { display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginTop: 16, padding: 13, border: "1px solid #c7d7fe", borderRadius: 11, background: "#f5f8ff" } as const;
const compareActionStyle = { marginTop: 12, display: "flex", justifyContent: "flex-end" } as const;
const compareButtonStyle = { border: "1px solid #73aaf5", background: "#fff", color: "#0057b8", borderRadius: 9, padding: "8px 11px", fontWeight: 800, cursor: "pointer" } as const;
const selectedCompareButtonStyle = { ...compareButtonStyle, background: "#eaf3ff", borderColor: "#0057b8" } as const;
const compareHintStyle = { marginTop: 16, padding: 12, border: "1px dashed #73aaf5", borderRadius: 10, background: "#f7fbff", color: "#475467" } as const;
const comparisonPanelStyle = { marginTop: 22, padding: 18, border: "1px solid #b2ddff", borderRadius: 14, background: "#fff" } as const;
const comparisonHeaderStyle = { display: "grid", gridTemplateColumns: "230px repeat(3,minmax(240px,1fr))", borderBottom: "2px solid #d7e3f4" } as const;
const comparisonRowStyle = { display: "grid", gridTemplateColumns: "230px repeat(3,minmax(240px,1fr))", borderBottom: "1px solid #eaecf0" } as const;
const comparisonLabelStyle = { padding: 12, background: "#f8fafc", color: "#344054" } as const;
const comparisonCellStyle = { padding: 12, minWidth: 0, lineHeight: 1.4 } as const;
const comparisonCourseHeaderStyle = { display: "grid", gap: 5, padding: 12, alignContent: "start" } as const;
const removeCompareButtonStyle = { justifySelf: "start", border: 0, background: "transparent", color: "#b42318", padding: 0, fontWeight: 750, cursor: "pointer" } as const;
