...
type AIScore = { courseId: string; aiScore: number; eligibilityStatus: "likely_meets" | "needs_review" | "requirements_not_verified"; reasons: string[]; cautions: string[]; entryRequirement: EntryRequirement | null };
...
export default function QuickMatchPage() {
  ...
  const enrichWithAI = async (base: LiveRecommendation[], migrationImportance: MigrationImportance) => {
    ...
    if (!response.ok) throw new Error(data.detail || data.error || "Unable to calculate AI scores.");
    setAiMode(data.mode ?? "");
  ...
  return <main style={pageStyle}>
    <header style={{ marginBottom: 24 }}>
      <div style={topRowStyle}><span style={badgeStyle}>Quick Match · AI + live database</span><button type="button" onClick={reset} style={ghostButtonStyle}>Clear saved answers</button></div>
      <h1 style={{ margin: "16px 0 8px", fontSize: 42, color: "#fff" }}>What should I study in Australia?</h1>
      <p style={heroCopyStyle}>UniPath first finds live course matches, then uses AI to score fit using your study background, career goal, budget, location and source-backed entry requirements.</p>
    </header>
    ...
    {stage === "input" && <section style={shellStyle}>
      ...
      <div style={footerStyle}><button type="button" disabled={step === 1} onClick={() => setStep(Math.max(1, step - 1) as QuickStep)} style={secondaryButtonStyle}>← Back</button>{step < 4 ? <button type="button" onClick={() => setStep((step + 1) as QuickStep)} style={primaryButtonStyle}>Continue →</button> : <button type="button" onClick={showQuickResult} style={primaryButtonStyle}>Get AI Match Score →</button>}</div>
    </section>}

    {(stage === "result" || stage === "detailed-result") && <><section style={sectionStyle}><div style={eyebrowStyle}>AI-SCORED LIVE RESULT</div><h2>Your best matches</h2><p style={mutedStyle}>The AI score is a decision-support score, not an admission guarantee. Missing course requirements are shown as unverified rather than guessed.</p>{aiMessage && <div style={infoBannerStyle}><strong>{aiMode === "openai" ? "AI scoring active" : "Transparent fallback scoring"}:</strong> {aiMessage}</div>}{loading ? <p>Finding live courses and calculating AI scores…</p> : error ? <ErrorBox text={error} /> : <ResultCards results={results} />}</section>{stage === "result" && <section style={{ ...sectionStyle, marginTop: 16 }}><h2>Want a more detailed result?</h2><p style={mutedStyle}>Add funds, experience, skills, dependants and transport preferences without losing your Quick Match answers.</p><button type="button" onClick={() => setStage("detailed")} style={primaryButtonStyle}>Continue to Detailed Assessment</button></section>}<MigrationPrompt choice={migrationChoice} setChoice={setMigrationChoice} onContinue={showMigrationResult} /></>}

    {stage === "detailed" && <section style={sectionStyle}><div style={eyebrowStyle}>DETAILED ASSESSMENT</div><h2>Add more information</h2><div style={gridStyle}><CurrencyBudgetInput label="Total funds available" audCents={profile.totalFundsCents} onAudCentsChange={(totalFundsCents) => setProfile((c) => ({ ...c, totalFundsCents }))} /><label style={labelStyle}>Years of relevant experience<input type="number" min={0} max={40} step={0.5} value={profile.yearsExperience ?? 0} onChange={(e) => setProfile((c) => ({ ...c, yearsExperience: Number(e.target.value) }))} style={inputStyle} /></label><label style={labelStyle}>Skills<input value={(profile.skills ?? []).join(", ")} onChange={(e) => setProfile((c) => ({ ...c, skills: e.target.value.split(",").map((v) => v.trim()).filter(Boolean) }))} style={inputStyle} /></label><label style={labelStyle}>Dependants<input type="number" min={0} max={10} value={profile.dependants ?? 0} onChange={(e) => setProfile((c) => ({ ...c, dependants: Number(e.target.value) }))} style={inputStyle} /></label></div><div style={footerStyle}><button type="button" onClick={() => setStage("result")} style={secondaryButtonStyle}>Back</button><button type="button" onClick={showDetailedResult} style={primaryButtonStyle}>Recalculate AI Score</button></div></section>}

    {stage === "migration-result" && <section style={sectionStyle}><div style={eyebrowStyle}>MIGRATION-AWARE AI COMPARISON</div><h2>Migration-aware ranking</h2><p style={mutedStyle}>This stays separate from your original result. UniPath does not guarantee PR, visa eligibility, invitation or skills assessment.</p>{loading ? <p>Calculating…</p> : error ? <ErrorBox text={error} /> : <ResultCards results={migrationResults} />}<div style={warningStyle}>Migration evidence is used only where source-backed data exists. Missing evidence receives no invented advantage.</div><div style={footerStyle}><button type="button" onClick={() => setStage("result")} style={secondaryButtonStyle}>Back to original result</button><Link href="/local-v2/migration" style={linkButtonStyle}>Open Migration Explorer</Link></div></section>}
  </main>;
}
...
function ResultCards({ results }: { results: LiveRecommendation[] }) {
  ...
  return <div style={{ display: "grid", gap: 16, marginTop: 18 }}>{results.slice(0, 8).map((item, index) => {
    ...
    return <article key={item.course.id} style={cardStyle}><div style={topRowStyle}><div><div style={rankStyle}>#{index + 1} {index === 0 ? "Best AI match" : "Alternative"}</div><h3 style={{ fontSize: 23, margin: "6px 0" }}>{item.course.name}</h3><div style={{ fontWeight: 800, color: "#0057b8" }}>{item.university.name}</div><div style={mutedStyle}>{item.campus.name}{item.campus.city ? ` · ${item.campus.city}` : ""}{item.campus.state ? `, ${item.campus.state}` : ""} {item.campus.regional ? "· Regional" : ""}</div></div><div style={scoreStyle}><div style={{ fontSize: 11 }}>AI MATCH</div>{score}%</div></div>
      <div style={status === "likely_meets" ? successStyle : status === "needs_review" ? warningStyle : neutralStyle}><strong>{statusLabel}</strong>{item.ai?.entryRequirement?.source_url && <a href={item.ai.entryRequirement.source_url} target="_blank" rel="noreferrer" style={{ marginLeft: 8 }}>source ↗</a>}</div>
      <div style={scoreGridStyle}><span>Course relevance <strong>{item.scores.academic}%</strong></span><span>Career <strong>{item.scores.career}%</strong></span><span>Budget <strong>{item.scores.affordability}%</strong></span><span>Location <strong>{item.scores.location}%</strong></span><span>Base score <strong>{item.scores.overall}%</strong></span></div>
      ...
      {item.ai && (item.ai.reasons.length > 0 || item.ai.cautions.length > 0) && <div style={reasonStyle}><strong>AI explanation</strong>{item.ai.reasons.length > 0 && <ul>{item.ai.reasons.map((r) => <li key={r}>{r}</li>)}</ul>}{item.ai.cautions.length > 0 && <><strong>Check before applying</strong><ul>{item.ai.cautions.map((r) => <li key={r}>{r}</li>)}</ul></>}</div>}
